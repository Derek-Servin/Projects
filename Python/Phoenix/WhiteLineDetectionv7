...
def process_video(video_path):
    ...
    while True:
        if not paused:
            ...
            '''            
            # Polynomial Fitting and Overlay on Original Frame
            points = np.column_stack(np.where(white_mask > 0))  # Extract non-zero points (y, x)
            '''

            # Polynomial Fitting and Overlay on Original Frame
            if largest_contour is not None:
                # Extract x and y points from the cyan contour
                contour_points = np.squeeze(largest_contour)  # Remove redundant dimensions
                if len(contour_points.shape) == 2 and contour_points.shape[0] > 10:  # Ensure valid points
                    x = contour_points[:, 0]  # x-coordinates
                    y = contour_points[:, 1]  # y-coordinates

                    # Fit a 2nd-degree polynomial (quadratic) to the contour points
                    try:
                        polynomial_coeffs = np.polyfit(x, y, 2)
                        polynomial_curve = np.poly1d(polynomial_coeffs)

                        '''
                        # Generate x-values for the curve within the image width
                        x_range = np.linspace(0, width, num=500)
                        ...
                        # Restrict the x-range to the contour bounds, capped at half the frame width
                        x_start = max(x_min, 0)
                        x_end = min(x_max, width)
                        x_range = np.linspace(x_start, x_end, num=500)

                        # Evaluate the polynomial within this restricted range
                        y_range = polynomial_curve(x_range)


                        # Overlay the polynomial curve onto a copy of the original frame
                        frame_with_poly = frame_resized.copy()
                        for (x_val, y_val) in zip(x_range, y_range):
                            if 0 <= int(y_val) < height and 0 <= int(x_val) < width:
                                cv2.circle(frame_with_poly, (int(x_val), int(y_val)), 2, (0, 0, 255), -1)  # Red curve
                    except np.linalg.LinAlgError:
                        frame_with_poly = frame_resized.copy()  # Fallback if fitting fails
                else:
                    frame_with_poly = frame_resized.copy()  # No polynomial if insufficient points
            else:
                frame_with_poly = frame_resized.copy()  # No polynomial if no valid contour


            # Display the original frame with cyan contours
            cv2.imshow("Lane Marking with Fitted Polynomial", frame_resized)

            # Display the white mask
            cv2.imshow("White Mask", white_mask)

            # Display the frame with the fitted polynomial in a new window
            cv2.imshow("Polynomial Overlay", frame_with_poly)
